...
experiment = "./results/segments_random_baseline_6000_dev_yoav"
...
try:
    test_dataset = DatasetParser.parse("data/nav_drone/dev_annotations_6000.json",
                                       config)
    logging.info("test dataset size: %d" % len(test_dataset))
    logging.info("RANDOM BASELINE")
    random_baseline = RandomBaseline(server=server,
                                     action_space=action_space,
                                     meta_data_util=meta_data_util,
                                     config=config,
                                     constants=constants)

    random_baseline.test_baseline(test_dataset)

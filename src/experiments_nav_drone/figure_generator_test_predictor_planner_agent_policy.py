...
from agents.figure_generator_predicter_planner_agent import FigureGeneratorPredictorPlannerAgent
...
experiment = "./results/figure_generator_dev"
...
try:
    ...
    tmp_config = config
    tmp_config["image_height"] = 128
    tmp_config["image_width"] = 128
    print("Tmp config is ", tmp_config)
    predictor_model = IncrementalModelAttentionChaplotResNet(tmp_config, constants,
    ...
    planner_model = IncrementalModelOracleGoldProb(tmp_config, constants, use_image=False)
    ...
    agent = FigureGeneratorPredictorPlannerAgent(server=server,

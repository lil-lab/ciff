...
class ResnetImageDetection(object):
...
class ImageRyanDetectionModule(nn.Module):
    ...
    def forward(self, image_batch):
        ...
        image_module_output = image_module_output.view(batch_size * NO_BUCKETS, 32)

        x_theta = self.dense_theta(image_module_output).contiguous()  # (batchx12) x 67
        x_theta = x_theta.view(batch_size, NO_BUCKETS, NUM_LANDMARKS) # batch x 12 x 67
        x_theta = x_theta.transpose(1, 2)  # batch x 67 x 12
        x_theta = x_theta.contiguous().view(batch_size * NUM_LANDMARKS, NO_BUCKETS)
        # x_theta = F.log_softmax(x_theta)  # (batch x 67) x 12

        return x_theta.view(batch_size, NUM_LANDMARKS, NO_BUCKETS)

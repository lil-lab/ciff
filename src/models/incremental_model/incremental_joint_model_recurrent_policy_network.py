...
from models.incremental_module.incremental_joint_multimodal_recurrent_simple_module import \
    IncrementalJointMultimodalRecurrentSimpleModule
...
from models.module.symbolic_instruction_module_probabilities import SymbolicInstructionModuleProbabilities
...
class IncrementalJointModelRecurrentPolicyNetworkSymbolicTextResnet(AbstractIncrementalModel):
    def __init__(self, config, constants, angle_prediction_model, symbolic_text_model):
        ...
        self.text_module = SymbolicInstructionModuleProbabilities(
        ...
        self.symbolic_text_model = symbolic_text_model
        ...
        final_module = IncrementalJointMultimodalRecurrentSimpleModule(
            image_module=self.image_module,
            ...
            landmark_model=symbolic_text_model)
    ...
    def get_probs(self, agent_observed_state, model_state, mode=None, volatile=False):
        ...
        start_images = [aos.data_point.get_six_start_images() for aos in agent_observed_state_list]
        instructions = [aos.get_instruction()
                        for aos in agent_observed_state_list]
        read_pointers = [aos.get_read_pointers()
                         for aos in agent_observed_state_list]
        ...
        probs_batch, new_model_state, image_emb_seq, state_feature = self.final_module(image_batch,
    ...
    def load_saved_model(self, load_dir_core):

        self.angle_prediction_model.save_model(load_dir_core + "_angle_model")
        self.symbolic_text_model.save_model(load_dir_core + "_symbolic_model")
        load_dir = load_dir_core + "_joint_model"
    ...
    def save_model(self, save_dir_core):

        self.angle_prediction_model.save_model(save_dir_core + "_angle_model")
        self.symbolic_text_model.save_model(save_dir_core + "_symbolic_model")

        save_dir = save_dir_core + "_joint_model"
    ...
    def get_parameters(self):
        ...
        parameters += self.angle_prediction_model.get_parameters()
        parameters += self.symbolic_text_model.get_parameters()
        if self.action_prediction_module is not None:
    ...
    def get_named_parameters(self):
        ...
        named_parameters += self.angle_prediction_model.get_named_parameters()
        named_parameters += self.symbolic_text_model.get_named_parameters()
        if self.action_prediction_module is not None:

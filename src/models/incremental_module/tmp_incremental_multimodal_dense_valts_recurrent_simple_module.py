...
class TmpIncrementalMultimodalDenseValtsRecurrentSimpleModule(nn.Module):
    """
    ...
    def __init__(self, image_module, text_module, action_module,
        ...
        super(TmpIncrementalMultimodalDenseValtsRecurrentSimpleModule, self).__init__()
        ...
        self.dense_block = nn.Linear(hidden_dim, 20)
        self.dense_direction = nn.Linear(hidden_dim, 5)
    ...
    def forward(self, image, image_lens, instructions, prev_action, mode, model_state):
        ...
        if mode is None or mode == ReadPointerAgent.ACT_MODE:
            ...
            direction_logprob = F.log_softmax(direction_logits)  # 1 x num_direction

...
class TmpBlocksIncrementalMultimodalChaplotModule(nn.Module):
    """
    ...
    def __init__(self, image_module, text_module, image_recurrence_module,
                 max_episode_length):
        super(TmpBlocksIncrementalMultimodalChaplotModule, self).__init__()
        ...
        self.blocks_layer = nn.Linear(256, 20)
        self.directions_layer = nn.Linear(256, 5)
    ...
    def forward(self, image, instructions, tx, mode, model_state):
        ...
        blocks_logit = self.blocks_layer(x)
        direction_logit = self.directions_layer(x)
        ...
        if mode is None or mode == ReadPointerAgent.ACT_MODE:
            return action_logprob, new_model_state, image_emb_seq, x

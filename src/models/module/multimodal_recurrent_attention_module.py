...
class MultimodalRecurrentAttentionModule(nn.Module):
    """
    pytorch module for final part of model
    combines embeddings of image, text, and previous action
    """
    def __init__(self, image_module, image_recurrence_module,
                 text_module, action_module,
                 total_emb_size, num_actions):
        super(MultimodalRecurrentAttentionModule, self).__init__()
        self.image_module = image_module
        self.image_recurrence_module = image_recurrence_module
        self.text_module = text_module
        self.action_module = action_module
        self.dense1 = nn.Linear(total_emb_size, 120)
        self.dense2 = nn.Linear(120, num_actions)
        self.dense_read = nn.Linear(120, 2)
    ...
    def forward(self, image, image_lens, instructions, prev_action, mode):
        ...
        x = torch.cat([image_emb, action_emb], dim=1)

...
class FigureGeneratorPredictorPlannerAgent:
    ...
    def save_attention_prob(self, image, attention_prob, instruction, scene_name, scale=6):
        self.global_id += 1

        image_flipped = image.swapaxes(0, 1).swapaxes(1, 2)
        image_flipped = scipy.misc.imresize(image_flipped, (128 * scale, 128 * scale * 6))
        attention_prob = attention_prob[:-1].view(32, 192).cpu().data.numpy()
        resized_kernel = scipy.misc.imresize(attention_prob, (128 * scale, 128 * scale * 6))

        plt.title(instruction)
        plt.imshow(image_flipped)
        plt.imshow(resized_kernel, cmap='jet', alpha=0.5)
        plt.savefig("./failure_examples/" + str(scene_name) + ".png")
        plt.clf()

    def save_example(self, image, instruction, scene_name, scale=6):
        self.global_id += 1

        image_flipped = image.swapaxes(0, 1).swapaxes(1, 2)
        image_flipped = scipy.misc.imresize(image_flipped, (128 *scale, 128 * scale))

        plt.title("\n".join(wrap(instruction, 60)))
        plt.imshow(image_flipped)
        plt.savefig("./dev_examples/" + str(scene_name) + ".png")
        plt.clf()
    ...
    def _test(self, data_point_ix, data_point, test_image, tensorboard=None, debug=False):
        ...
        self.save_attention_prob(high_quality_test_image_example, attention_prob, info["instruction_string"],
                                 info["datapoint_id"])
    ...
    def test(self, test_dataset, tensorboard=None, logger=None,
        ...
        indices = []
        good_examples = ["scene_0641_01", "scene_1727_04", "scene_2348_01", "scene_3539_03",
                              "scene_3720_08", "scene_3926_03", "scene_4937_00", "scene_5213_04"]
        bad_examples = ["scene_1234_06", "scene_2008_02", "scene_3718_02", "scene_4336_02",
                        "scene_4894_05", "scene_5070_02", "scene_5622_03", "scene_5624_00"]

        chosen_scene_names = bad_examples

        for ix, data_point in enumerate(test_dataset):
            if data_point.get_scene_name() in chosen_scene_names:
                indices.append(ix)

        assert len(chosen_scene_names) == len(indices)

        new_test_dataset = []
        new_test_image = []
        for i in indices:
            new_test_dataset.append(test_dataset[i])
            new_test_image.append(test_image[i])

        test_dataset = new_test_dataset
        test_image = new_test_image

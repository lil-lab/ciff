...
def set_doom_configuration(game, params):
    ...
    # game.add_available_button(Button.TURN_LEFT, 15)
    # game.add_available_button(Button.TURN_RIGHT, 15)
    # game.add_available_button(Button.MOVE_FORWARD, 1.5)
...
# GET ANGLE in degrees ADDED BY EYVIND
def get_agent_orientation(game):
    angle = game.get_game_variable(GameVariable.ANGLE)
    # Angle calculation below seem wrong so I commented it out --Dipendra
    # angle = get_world_coordinates(game.get_game_variable(GameVariable.ANGLE))
    return angle

...
class a3c_lstm_ga_concat_instructions(torch.nn.Module):

    def __init__(self, args, config=None):
        super(a3c_lstm_ga_concat_instructions, self).__init__()
        self.config = config
        sep = torch.from_numpy(np.array([config["vocab_size"]]))
        self.seperator = Variable(sep.cuda())
        ...
        self.gru.flatten_parameters() ## Added by Dipendra cause it was giving warning
    ...
    def forward(self, inputs, cached_computation=None):
        ...
        if cached_computation is None:
            ...
            # prev instruction
            for i in range(prev_instr.data.size(1)):
                word_embedding = self.embedding(prev_instr[0, i]).unsqueeze(0)
                _, encoder_hidden = self.gru(word_embedding, encoder_hidden)
            # seperator 1
            word_embedding = self.embedding(self.seperator[0]).unsqueeze(0)
            _, encoder_hidden = self.gru(word_embedding, encoder_hidden)
            # current instruction
            for i in range(curr_instr.data.size(1)):
                word_embedding = self.embedding(curr_instr[0, i]).unsqueeze(0)
                _, encoder_hidden = self.gru(word_embedding, encoder_hidden)
            # seperator 2
            word_embedding = self.embedding(self.seperator[0]).unsqueeze(0)
            _, encoder_hidden = self.gru(word_embedding, encoder_hidden)
            # next instruction
            for i in range(next_instr.data.size(1)):
                word_embedding = self.embedding(next_instr[0, i]).unsqueeze(0)
                _, encoder_hidden = self.gru(word_embedding, encoder_hidden)

...
class a3c_lstm_ga_directional(torch.nn.Module):

    def __init__(self, args, config=None):
        super(a3c_lstm_ga_directional, self).__init__()
        self.config = config
        ...
        # Create directional feature maps
        self.final_image_height = 6
        self.final_image_width = 6
        directional_maps_array = np.zeros((1, 3, self.final_image_height, self.final_image_width))
        column_width = (self.final_image_width + 2) // 3
        for i in range(self.final_image_height):
            for j in range(self.final_image_width):
                if j < column_width:
                    directional_maps_array[0, 0, i, j] = 1
                elif j < self.final_image_width - column_width:
                    directional_maps_array[0, 1, i, j] = 1
                else:
                    directional_maps_array[0, 2, i, j] = 1
        self.directional_maps = Variable(torch.from_numpy(directional_maps_array).float().cuda())
        ...
        self.attn_linear = nn.Linear(self.gru_hidden_size, 67)
        ...
        # A3C-LSTM layers
        self.linear = nn.Linear(67 * self.final_image_height * self.final_image_width, 256)
    ...
    def forward(self, inputs, cached_computation=None):
        ...
        x_image_rep = torch.cat([x_image_rep, self.directional_maps], dim=1)
        ...
        if cached_computation is None:
            ...
            x_attention = x_attention.expand(1, 67, self.final_image_height, self.final_image_width)

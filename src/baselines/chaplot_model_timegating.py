...
class a3c_lstm_ga_default(torch.nn.Module):

    def __init__(self, args, config=None):
        super(a3c_lstm_ga_default, self).__init__()
        self.config = config
        ...
        self.attn_linear = nn.Linear(self.gru_hidden_size + 32, 64)
    ...
    def forward(self, inputs, cached_computation=None):
        ...
        if cached_computation is None:
            ...
            cached_computation["x_instr_rep"] = x_instr_rep
        else:
            x_instr_rep = cached_computation["x_instr_rep"]

        # Get the attention vector from the instruction representation
        time_emb = self.time_emb_layer(tx).view(1, -1)
        x_attention_input = torch.cat([x_instr_rep, time_emb], dim=1)
        x_attention = F.sigmoid(self.attn_linear(x_attention_input))

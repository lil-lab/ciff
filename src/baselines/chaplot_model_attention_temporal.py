...
class a3c_lstm_ga_attention_temporal(torch.nn.Module):

    def __init__(self, args, config=None):
        super(a3c_lstm_ga_attention_temporal, self).__init__()
        self.config = config
        ...
        self.attention_key_prev = nn.Linear(256 * 3 + 32, self.attn_key_size)
        self.attention_key_next = nn.Linear(256 * 3 + 32, self.attn_key_size)
    ...
    def forward(self, inputs, cached_computation=None):
        ...
        attention_key_input = torch.cat([curr_instr_rep, image_emb, time_emb.view(1, -1)], dim=1)

        # attention for previous instructions
        ...
        # attention for next instructions

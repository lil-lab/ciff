...
def main(args):
    ...
    experiment_name = "test_house_chaplot_house_baseline_test_dataset_a3c_postbugfix"
    ...
    try:
        ...
        model.load_saved_model("./results/train_house_chaplot_house_baseline_postbugfix/chaplot_model_epoch_2")
        ...
        for hid in house_ids:
            ...
            test_split.append(all_dev_data)

            master_logger.log("Created test dataset of size {} ".format(len(all_dev_data)))
        ...
        for i, port in enumerate(ports):
            ...
            print("Client " + str(house_ids[i]) + " getting a test set of size ", len(test_chunk))
            server = HouseServer(tmp_config, action_space, port)
            client_logger = multiprocess_logging_manager.get_logger(i)

            # Run the Testing
            p = mp.Process(target=ChaplotBaselineHouse.do_test, args=(house_ids[i], model, tmp_config, action_space,
                                                                      meta_data_util, constants, test_chunk,

...
def main():
    ...
    experiment_name = "debug_human_control"
    ...
    # Create the server
    server = StreetViewServer(config, action_space)

    # Read the dataset
    test_data = DatasetParser.parse("data/streetview/navigation_test.json", config)

    agent = Agent(server, None, None, action_space, meta_data_util, config, constants)
    for dp in test_data:
        agent.debug_human_control(dp)
